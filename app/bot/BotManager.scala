...
object BotManager {
  ...
  def setCapacity(newCapacity: Double): Unit = {
    this.synchronized {
      ...
      bots.map(tuple => BuyBot(capacity, tuple._2.depleted, tuple._2.totalBought))
    }
  }

  def getBot(id: Int): BuyBot = {
    bots.get(id).orElse(disabled.get(id)).getOrElse[BuyBot]({
      val bot: BuyBot = new BuyBot(capacity, 0, 0)
      bots = bots.+((id, bot))
      bot
    })
  }
  ...
  def changeBehaviour(id: Int, newMax: Int): Unit = {
    this.synchronized {
      bots = bots.+((id, new BuyBot(newMax, bots.get(id).map(_.depleted).getOrElse(0), bots.get(id).map(_.totalBought).getOrElse(0))))
    }
  }
